...
#
# converts a system using ipadm and dhcp to a static network
...
#
# conventionally the main interface would be /v4
#
IP1=$(/usr/sbin/ipadm show-addr -p -o addr "${IFACE}/v4")
#
...
#
echo "/usr/sbin/ipadm delete-addr ${IFACE}/v4"
...
if [ -z "$DRYRUN" ]; then
    /usr/sbin/ipadm delete-addr "${IFACE}/v4"
